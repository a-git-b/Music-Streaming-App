        ...
        <div v-for="album in albums" :key="album.id" class="card my-card">
            ...
            <div class="card-body">
                ...
                <div class="btn-group" role="group">
                    <button @click="showSongs(album.id)" type="button" class="btn btn-secondary">View Songs</button>
                    <button @click="addSong(album.id)" type="button" class="btn btn-primary">Add Song</button>
                    <b-button @click="showEditAlbum(album.id)">Edit</b-button>
                    <button @click="deleteAlbum(album.id)" type="button" class="btn btn-primary">Delete</button>
...
export default {
    ...
    showSongs(album_id) {
        ...
        this.$router.push("/creatoralbumsongs/"+album_id)
    },
     addSong(album_id){
    ...
    async deleteAlbum(album_id){
        try{
            const response = await axios.delete('http://127.0.0.1:8000/deletealbum/'+album_id);
            console.log(response.data);
        }
        catch(error){
            console.error(error.response.data);
